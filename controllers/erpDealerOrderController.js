...
export const generateInvoice = async (req, res) => {
  try {
    ...
    const invoice = await Invoice.create({
      ...
      salesOrderId: order._id,
      // Party / Bill To = Dealer (the person who placed the order)
      ...
      billToName: order.dealerId?.businessName || order.dealerId?.name || order.customer,
      billToAddress: order.dealerId?.address || order.deliveryAddress || '',
      billToCity: order.dealerId?.city || '',
      billToState: order.dealerId?.state || '',
      billToPincode: order.dealerId?.pincode || '',
      billToGST: order.dealerId?.gstin || '',
      // Ship To = Sri Chakra Industries (the company dispatching the goods)
      shipToName: 'Sri Chakra Industries',
      shipToAddress: '#13/14, Azeez Sait Industrial Estate, Mysore Road, Nayandahalli, Bangalore - 560039',
      items,
      subtotal,
      totalTax,
      grandTotal,
