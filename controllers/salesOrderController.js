...
export const createOrder = async (req, res) => {
  try {
    const { customer, items, value, priority, status, orderDate, remarks } = req.body;
...
export const updateOrder = async (req, res) => {
  try {
    const { customer, items, value, priority, status, remarks } = req.body;
    const update = {};
    if (customer !== undefined) update.customer = customer;
    if (items !== undefined) update.items = parseInt(items) || 0;
