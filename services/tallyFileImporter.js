...
function parseVouchers(xml, voucherType) {
  ...
  for (const m of xml.matchAll(typePattern)) {
    ...
    // Parse inventory entries — Tally uses ALLINVENTORYENTRIES.LIST for Sales/Purchase
    ...
    for (const inv of block.matchAll(invPattern)) {
      ...
      const sn = (ib.match(/<STOCKITEMNAME>([\s\S]*?)<\/STOCKITEMNAME>/i)?.[1] || '').trim();
      if (!sn) continue;
      const rawQty  = (ib.match(/<BILLEDQTY>([\s\S]*?)<\/BILLEDQTY>/i)?.[1] || ib.match(/<ACTUALQTY>([\s\S]*?)<\/ACTUALQTY>/i)?.[1] || '0').trim();
      const rawRate = (ib.match(/<RATE>([\s\S]*?)<\/RATE>/i)?.[1] || '0').trim();
      const qty  = parseFloat(rawQty.replace(/[^\d.-]/g, ''))  || 0;
      const rate = parseFloat(rawRate.replace(/[^\d.-]/g, '')) || 0;
      const amt  = Math.abs(parseFloat(ib.match(/<AMOUNT>([\s\S]*?)<\/AMOUNT>/i)?.[1]) || 0);
      inventoryEntries.push({ stockItemName: sn, qty, rate, amount: amt });
    }

    // Compute amount: items+taxes > max ledger > fallback AMOUNT
    let amount = 0;
    if (inventoryEntries.length > 0) {
      ...
      const taxTotal  = ledgerEntries.reduce((s, l) => {
        ...
        return (n.includes('cgst') || n.includes('sgst') || n.includes('igst') || n.includes('gst') || n.includes('tax') || n.includes('round'))
          ? s + Math.abs(l.amount) : s;
      }, 0);
    ...
    if (amount === 0) amount = Math.abs(parseFloat(block.match(/<AMOUNT>([\s\S]*?)<\/AMOUNT>/i)?.[1]) || 0);

    vouchers.push({ guid, alterId, voucherNo, voucherType: vt || voucherType, partyName, amount, narration, vDate, ledgerEntries, inventoryEntries });
  }
  return vouchers;
}
      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            voucherDate: v.vDate, ledgerEntries: v.ledgerEntries, inventoryEntries: v.inventoryEntries || [],
...
export function startFileWatcher() {
  ...
  const triggerImport = () => {
    ...
    _debounceTimer = setTimeout(async () => {
      LOG('Files changed, starting import...');
      try {
        await importFromFiles();
        LOG('Import complete');
      } catch (err) {
        console.error('[TallyFileWatcher] Import failed:', err);
      }
    }, 3000);
  };
  ...
  _watcher
    ...
    .on('error', (error) => {
      console.error('[TallyFileWatcher] Watcher error:', error);
    });

  LOG(`Watching directory for changes: ${EXPORT_DIR}`);
}
...
export function stopFileWatcher() {
  if (_watcher) {
    _watcher.close();
    _watcher = null;
    console.log('[TallyFileWatcher] Stopped watching');
  }
}
